...
async function paypalConfig(env) { const mode = String(env.PAYPAL_MODE || "").toLowerCase();
return { clientId: env.PAYPAL_CLIENT_ID || "", mode }; }
async function createOrder(request, env) { try { const body = await request.json();
...
async function captureOrder(request, env) { try { const body = await request.json();
...
} catch (error) { console.error( "CAPTURE ORDER ERROR:", error );
return json(
  {
    error:
      error.message ||
...
async function health(env) { let database = "missing";
try { await env.DB.prepare( "SELECT 1" ).first();
database = "configured";
} catch (error) { console.error( "DATABASE HEALTH ERROR:", error ); }
return json({ ok: true, worker: "DealRank", paypal_mode: env.PAYPAL_MODE ? "configured" : "missing", paypal_client_id: env.PAYPAL_CLIENT_ID ? "configured" : "missing", paypal_secret: env.PAYPAL_CLIENT_SECRET ? "configured" : "missing", database }); }
